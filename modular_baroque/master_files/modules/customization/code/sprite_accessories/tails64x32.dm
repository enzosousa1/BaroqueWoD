//// PLEASE DO NOT SEPARATE FOLDERS. IT BECOMES VERY CLUTTERED. ty.

// Ported over from CHOMP
// https://github.com/CHOMPStation2/CHOMPStation2/pull/9926

// because we are just running skyratcode pretty much
// this system is so much worse than what they have on CHOMP apparently (lol)
// 🦈

/datum/sprite_accessory/nocturne/tails/baroque/largeshark
	icon = 'modular_baroque/master_files/modules/customization/icons/tails64x32.dmi'
	dimension_x = 64
	dimension_y = 32
	center = TRUE
	color_src = USE_MATRIXED_COLORS

/datum/sprite_accessory/nocturne/tails/baroque/largeshark/default
	name = "Large Shark"
	icon_state = "large-shark"

/datum/sprite_accessory/nocturne/tails/baroque/largeshark/finless
	name = "Large Shark (finless)"
	icon_state = "large-shark-finless"

/datum/sprite_accessory/nocturne/tails/baroque/largeshark/striped
	name = "Large Shark (striped)"
	icon_state = "large-shark-s"

/datum/sprite_accessory/nocturne/tails/baroque/largeshark/striped_finless
	name = "Large Shark (striped, finless)"
	icon_state = "large-shark-finless-s"

// you could definitely cook up more combinations and use the "stripes-underbelly" sprite but... yeah

/datum/sprite_accessory/nocturne/tails/baroque/bigandfluffy
	name = "Shadekin Big and Fluffy"
	icon = 'modular_baroque/master_files/modules/customization/icons/tails64x32.dmi'
	icon_state = "bigandfluffy"
	dimension_x = 64
	dimension_y = 32
	center = TRUE
	color_src = USE_MATRIXED_COLORS

/datum/sprite_accessory/nocturne/tails/baroque/longstalker
	name = "Hybrid Coyote (Large)"
	icon_state = "longstalker"
	icon = 'modular_baroque/master_files/modules/customization/icons/tails64x32.dmi'
	dimension_x = 64
	dimension_y = 32
	center = TRUE
	color_src = USE_MATRIXED_COLORS
