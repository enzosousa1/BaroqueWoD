...
/datum/sprite_accessory/clothing/proc/make_appearance(color = COLOR_WHITE, physique = MALE, bodyshape = BODYSHAPE_HUMANOID)
	...
	*/
	...
	var/key = "[icon_state_to_use][digi_icon_key]-[greyscale_config || "ng"]-[use_female]-[use_digi]-[greyscale_colors]" // NOCTURNE EDIT - ORIGINAL: var/key = "[icon_state_to_use]-[greyscale_config || "ng"]-[use_female]-[use_digi]-[greyscale_colors]"
	...
	if(cached_icons[key]) // it's already cached
		...
		var/icon/created = icon(greyscale_config ? SSgreyscale.GetColoredIconByType(greyscale_config, greyscale_colors) : icon, "[icon_state_to_use][digi_icon_key]") // NOCTURNE EDIT - ORIGINAL: var/icon/created = icon(greyscale_config ? SSgreyscale.GetColoredIconByType(greyscale_config, greyscale_colors) : icon, icon_state_to_use)
		if(use_female)
			created = wear_female_version("[icon_state_to_use][digi_icon_key]", icon, female_sprite_flags_to_use) // NOCTURNE EDIT - ORIGINAL: created = wear_female_version(icon_state_to_use, icon, female_sprite_flags_to_use)
		if(use_digi)
